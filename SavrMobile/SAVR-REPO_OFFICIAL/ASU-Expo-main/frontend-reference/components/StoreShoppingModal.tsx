                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-muted-foreground truncate sm:whitespace-normal sm:break-words">
                      {selection?.brand && <>{decodeHTMLEntities(selection.brand)} · </>}
                      {selection?.size}
                    </div>
                  ...
                  <div className="ml-auto text-right">
                    ...
                    {(() => {
                      ...
                      if (qty <= 1 || !selection || selection.name === 'Nothing') return null;
                      const priceStr = selection.price;
                      if (!priceStr) return null;
                      const n = typeof priceStr === 'number' ? priceStr : parseFloat(String(priceStr).replace(/[$£€]/g, '').trim()) || 0;
                      if (n <= 0) return null;
                      return (
                        <div className="text-xs font-medium text-primary">
                          {qty} x ${n.toFixed(2)} = ${(n * qty).toFixed(2)}
                        </div>
                      );
                    })()}
                  </div>
...
const StoreShoppingModal: React.FC<StoreShoppingModalProps> = ({
  ...
  const subtotal = useMemo(() => {
    return itemsForStore.reduce((sum, { item, selection }) => {
  ...
  const handlePdf = async () => {
    try {
      ...
      toast({ title: 'Generating PDF...', variant: 'info' });
