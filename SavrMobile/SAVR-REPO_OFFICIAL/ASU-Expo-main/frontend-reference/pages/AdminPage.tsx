...
const Section: React.FC<{ title: string, open?: boolean, children: React.ReactNode }> = ({ title, open = false, children }) => {
  ...
  return (
    <div className="border rounded-lg mb-3 bg-white overflow-visible shadow-sm">
      <button className="w-full flex items-center justify-between px-4 py-3 font-bold bg-gray-50 hover:bg-gray-100 transition-colors rounded-t-lg" onClick={() => setIsOpen(!isOpen)}>
        <span>{title}</span>
        <svg className={`w-4 h-4 text-gray-500 transition-transform duration-200 ${isOpen ? 'rotate-90' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
      </button>
      {isOpen && (
  ...
  return (
    <div className="p-4 max-w-6xl mx-auto">
      ...
      <Section title="Dashboard" open>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="border rounded-lg p-3 border-l-4 border-l-green-500">
            <div className="text-gray-500 text-xs font-medium uppercase tracking-wide">Total Users</div>
            <div className="text-2xl font-extrabold mt-1">{overview?.total_users ?? '—'}</div>
          </div>
          <div className="border rounded-lg p-3 border-l-4 border-l-green-500">
            <div className="text-gray-500 text-xs font-medium uppercase tracking-wide">New Signups (This Month)</div>
            <div className="text-2xl font-extrabold mt-1">{overview?.new_signups_this_month ?? '—'}</div>
          </div>
          <div className="border rounded-lg p-3 border-l-4 border-l-green-500">
            <div className="text-gray-500 text-xs font-medium uppercase tracking-wide">Visitors Today</div>
            <div className="text-2xl font-extrabold mt-1">{overview?.visitors_today ?? '—'}</div>
          </div>
          <div className="border rounded-lg p-3 border-l-4 border-l-green-500">
            <div className="text-gray-500 text-xs font-medium uppercase tracking-wide">Currently Online</div>
            <div id="admin-kpi-online" className="text-2xl font-extrabold mt-1">{online?.distinct_users ?? '—'}</div>
          </div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mt-3">
          <div className="border rounded-lg p-3 border-l-4 border-l-green-500">
            <div className="text-gray-500 text-xs font-medium uppercase tracking-wide">Total Lists</div>
            <div className="text-2xl font-extrabold mt-1">{overview?.total_grocery_lists ?? '—'}</div>
          </div>
          <div className="border rounded-lg p-3 border-l-4 border-l-green-500">
            <div className="text-gray-500 text-xs font-medium uppercase tracking-wide">Total Savings (All Time)</div>
            <div className="text-2xl font-extrabold mt-1">{fmt(overview?.total_savings_amount)}</div>
          </div>
          <div className="border rounded-lg p-3 border-l-4 border-l-green-500">
            <div className="text-gray-500 text-xs font-medium uppercase tracking-wide">Avg Savings / Active User</div>
            <div className="text-2xl font-extrabold mt-1">{fmt(overview?.average_savings_per_user)}</div>
          </div>
          <div className="border rounded-lg p-3 border-l-4 border-l-green-500">
            <div className="text-gray-500 text-xs font-medium uppercase tracking-wide">This Month's Savings</div>
            <div className="text-2xl font-extrabold mt-1">{fmt(overview?.this_month_savings)}</div>
          </div>
        </div>
      ...
      <Section title="Analytics">
        <AdminAnalyticsChart excludeAdmins={excludeAdmins} />
      </Section>
...
const UsersPanel: React.FC = () => {
  ...
  const load = async () => {
    try {
      ...
      params.set('sort_by', userSort.key);
      params.set('sort_dir', userSort.dir);
  ...
  // Load when page, pageSize, or sort changes
  useEffect(() => {
    load();
  }, [page, pageSize, userSort]);
  ...
  // Pagination calculations (server handles sorting and pagination)
  const totalPages = Math.max(1, Math.ceil(totalUsers / pageSize));
  const currentPageUsers = Array.isArray(rows) ? rows : [];
    ...
    <div className="space-y-3">
        ...
        <table className="w-full">
          <thead>
            <tr className="text-left text-xs uppercase tracking-wide text-gray-500">
              <th className={`p-2 cursor-pointer select-none whitespace-nowrap hover:text-gray-900 transition-colors ${userSort.key === 'username' ? 'text-gray-900 bg-gray-50' : ''}`} onClick={()=>setUserSortKey('username')}>Username{userSortIndicator('username')}</th>
              <th className={`p-2 cursor-pointer select-none whitespace-nowrap hover:text-gray-900 transition-colors ${userSort.key === 'status' ? 'text-gray-900 bg-gray-50' : ''}`} onClick={()=>setUserSortKey('status')}>Status{userSortIndicator('status')}</th>
              <th className={`p-2 cursor-pointer select-none whitespace-nowrap hover:text-gray-900 transition-colors ${userSort.key === 'online' ? 'text-gray-900 bg-gray-50' : ''}`} onClick={()=>setUserSortKey('online')}>Online{userSortIndicator('online')}</th>
              <th className={`p-2 cursor-pointer select-none whitespace-nowrap hover:text-gray-900 transition-colors ${userSort.key === 'last_login' ? 'text-gray-900 bg-gray-50' : ''}`} onClick={()=>setUserSortKey('last_login')}>Last Login{userSortIndicator('last_login')}</th>
              <th className={`p-2 cursor-pointer select-none whitespace-nowrap hover:text-gray-900 transition-colors ${userSort.key === 'login_count' ? 'text-gray-900 bg-gray-50' : ''}`} onClick={()=>setUserSortKey('login_count')}>Login Count{userSortIndicator('login_count')}</th>
              <th className={`p-2 cursor-pointer select-none whitespace-nowrap hover:text-gray-900 transition-colors ${userSort.key === 'lists_count' ? 'text-gray-900 bg-gray-50' : ''}`} onClick={()=>setUserSortKey('lists_count')}>Lists{userSortIndicator('lists_count')}</th>
              <th className={`p-2 cursor-pointer select-none whitespace-nowrap hover:text-gray-900 transition-colors ${userSort.key === 'total_savings' ? 'text-gray-900 bg-gray-50' : ''}`} onClick={()=>setUserSortKey('total_savings')}>Total Savings{userSortIndicator('total_savings')}</th>
              <th className="p-2 w-8"><span className="sr-only">Actions</span></th>
            </tr>
          </thead>
          <tbody>
            {currentPageUsers.map(r => (
              <tr key={r.id} className="border-t cursor-pointer hover:bg-gray-50 transition-colors" onClick={()=>selectUser(r.id)}>
                ...
                <td className="p-2">
                  <div className="relative inline-block text-left">
                    ...
                    {openMenuUserId === r.id && (
                      <div className="absolute right-0 mt-1 w-44 bg-white border rounded-lg shadow-lg z-10 py-1" role="menu" onClick={(e)=>e.stopPropagation()}>
                        <button className="block w-full text-left px-3 py-2 text-sm hover:bg-gray-50 transition-colors" onClick={(e)=>{ e.stopPropagation(); action(r.id,'reset'); setOpenMenuUserId(null); }}>Reset Password</button>
                        {r.is_active ? (
                          <button className="block w-full text-left px-3 py-2 text-sm hover:bg-gray-50 transition-colors" onClick={(e)=>{ e.stopPropagation(); action(r.id,'deactivate'); setOpenMenuUserId(null); }}>Deactivate</button>
                        ) : (
                          <button className="block w-full text-left px-3 py-2 text-sm hover:bg-gray-50 transition-colors" onClick={(e)=>{ e.stopPropagation(); action(r.id,'reactivate'); setOpenMenuUserId(null); }}>Reactivate</button>
                        )}
                        <button className="block w-full text-left px-3 py-2 text-sm hover:bg-gray-50 transition-colors" onClick={(e)=>{ e.stopPropagation(); action(r.id,'force'); setOpenMenuUserId(null); }}>Force Logout</button>
                        <div className="border-t my-1"></div>
                        <button className="block w-full text-left px-3 py-2 text-sm hover:bg-red-50 text-red-600 transition-colors" onClick={(e)=>{ e.stopPropagation(); action(r.id,'delete'); setOpenMenuUserId(null); }}>Delete</button>
                      </div>
                    )}
                  </div>
                </td>
      ...
      <div className="hidden md:flex flex-wrap items-center gap-2 mt-2">
        <button
          ...
          disabled={page === 0}
          className="border rounded px-2 py-1 text-sm hover:bg-gray-50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed disabled:hover:bg-transparent"
        >
          ← Prev
        </button>
  ...
  return (
    <div>
      <table className="w-full">
        <thead>
          <tr className="text-left text-xs uppercase tracking-wide text-gray-500">
            <th className="p-2 cursor-pointer select-none hover:text-gray-900 transition-colors" onClick={()=>setSortKey('store')}>Store{sortIndicator('store')}</th>
            <th className="p-2 cursor-pointer select-none hover:text-gray-900 transition-colors" onClick={()=>setSortKey('wins')}>Wins{sortIndicator('wins')}</th>
            <th className="p-2 cursor-pointer select-none hover:text-gray-900 transition-colors" onClick={()=>setSortKey('lists')}>Lists{sortIndicator('lists')}</th>
            <th className="p-2 cursor-pointer select-none hover:text-gray-900 transition-colors" onClick={()=>setSortKey('win_rate')}>Win-Rate{sortIndicator('win_rate')}</th>
            <th className="p-2 cursor-pointer select-none hover:text-gray-900 transition-colors" onClick={()=>setSortKey('avg_list_price')}>Avg List Price{sortIndicator('avg_list_price')}</th>
          </tr>
        </thead>
        <tbody>
          {sortedRows.map((r, i) => (
            <tr key={i} className="border-t hover:bg-gray-50 transition-colors"><td className="p-2">{r.store}</td><td className="p-2">{r.wins ?? 0}</td><td className="p-2">{r.lists ?? 0}</td><td className="p-2">{r.win_rate}%</td><td className="p-2">{r.avg_list_price ? `$${r.avg_list_price.toFixed(2)}` : '—'}</td></tr>
          ))}
        </tbody>
      </table>
    </div>
...
const StoreReliability: React.FC<{ excludeAdmins?: boolean }> = ({ excludeAdmins = false }) => {
  ...
  return (
    <div className="space-y-2">
      ...
      <div>
        <table className="w-full">
          <thead><tr className="text-left text-xs uppercase tracking-wide text-gray-500"><th className="p-2">Store</th><th className="p-2">Results Observed</th></tr></thead>
          <tbody>
            {data?.per_store?.map((r, i) => (
              <tr key={i} className="border-t hover:bg-gray-50 transition-colors"><td className="p-2">{r.store}</td><td className="p-2">{r.results}</td></tr>
            ))}
          </tbody>
        </table>
      </div>
    ...
    <div className="border rounded-lg p-3">
      ...
      <div>
        <table className="w-full">
          <thead>
            <tr className="text-left text-xs uppercase tracking-wide text-gray-500">
              <th className="p-2 cursor-pointer select-none hover:text-gray-900 transition-colors" onClick={()=>setSortKey('item')}>Item</th>
              <th className="p-2 cursor-pointer select-none hover:text-gray-900 transition-colors" onClick={()=>setSortKey('count')}>Count</th>
              <th className="p-2 cursor-pointer select-none hover:text-gray-900 transition-colors" onClick={()=>setSortKey('avg_price')}>Avg Price</th>
            </tr>
          </thead>
          <tbody>
            {current.map((r, i) => (
