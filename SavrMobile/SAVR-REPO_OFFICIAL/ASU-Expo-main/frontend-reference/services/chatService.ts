...
export interface GroceryItem {
  ...
  unit: string;
  checked?: boolean;
  priceQty?: number; // User-controlled quantity for price calculations (default 1)
}
...
const chatService = {
  ...
  sendMessage: async (data: ChatRequest): Promise<ChatResponse> => {
    try {
      ...
      try {
        ...
        console.warn('Streaming fallback to non-stream request due to error:');
      }
      ...
      console.error('Error in chatService.sendMessage:');
      ...
      fetchEventSource(url, {
        ...
        onmessage: (msg: EventSourceMessage) => {
          ...
          try {
            ...
            console.warn('Failed to parse SSE data chunk, ignoring.');
          }
        },
        ...
        onerror: (err) => {
          console.error('Stream transport error');
          reject(err);
  ...
  getWelcomeMessage: async (): Promise<WelcomeMessageResponse> => {
    try {
      ...
      console.error('Error fetching welcome message:');
...
export const onboardingChatService = {
  ...
  getWelcomeMessage: async (): Promise<WelcomeMessageResponse> => {
    try {
      ...
      console.error('Onboarding welcome error:');
      return {
        content:
